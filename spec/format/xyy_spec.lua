...
describe("clamp_xyy", function()
  it("clamps xyY values to normalized range [0, 1]", function()
    local x, y, Y = Format.clamp_xyy(0.5, 0.4, 0.8)
  ...
  it("handles boundary values", function()
    local x1, y1, Y1 = Format.clamp_xyy(0, 0, 0)
    local x2, y2, Y2 = Format.clamp_xyy(1, 1, 1)
  ...
  it("clamps out-of-range values", function()
    local x, y, Y = Format.clamp_xyy(-0.1, 1.5, 0.5)
  ...
  it("defaults Y to 1 when nil", function()
    local x, y, Y = Format.clamp_xyy(0.3, 0.6, nil)
  ...
  it("clamps Y when provided", function()
    local x, y, Y = Format.clamp_xyy(0.3, 0.6, 1.2)
    assert.are.equal(0.3, x)
    assert.are.equal(0.6, y)
    assert.are.equal(1, Y)
  end)
