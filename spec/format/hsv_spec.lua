...
describe("hsv_to_hsv360", function()
  it("converts normalized HSV to degrees", function()
    local h, s, v = HSV.hsv_to_hsv360(0.25, 0.8, 0.6)  -- 0.25 * 360 = 90
    assert.are.equal(90, h)
    assert.are.equal(0.8, s)
    assert.are.equal(0.6, v)
  end)
  ...
  it("wraps hue values", function()
    local h, s, v = HSV.hsv_to_hsv360(1.25, 0.5, 0.7)  -- 1.25 * 360 = 450, 450 % 360 = 90
    assert.are.equal(90, h)
    assert.are.equal(0.5, s)
    assert.are.equal(0.7, v)
  end)
...
describe("hsv360_to_hsv", function()
  it("converts degrees HSV to normalized", function()
    local h, s, v = HSV.hsv360_to_hsv(90, 0.8, 0.6)  -- 90 / 360 = 0.25
  ...
  it("wraps hue values", function()
    local h, s, v = HSV.hsv360_to_hsv(450, 0.5, 0.7)  -- 450 % 360 = 90, 90 / 360 = 0.25
    assert.are.equal(0.25, h)
    assert.are.equal(0.5, s)
    assert.are.equal(0.7, v)
  end)
