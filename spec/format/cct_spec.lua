...
describe("clamp_kelvin", function()
  it("clamps values to SmartThings Kelvin range [1, 30000]", function()
    local k = Format.clamp_kelvin(0)
    assert.are.equal(1, k)
  end)

  it("handles boundary values", function()
    local k1 = Format.clamp_kelvin(1)
    local k2 = Format.clamp_kelvin(30000)
    assert.are.equal(1, k1)
    assert.are.equal(30000, k2)
  end)

  it("clamps out-of-range values", function()
    local k1 = Format.clamp_kelvin(0)
    local k2 = Format.clamp_kelvin(40000)
    assert.are.equal(1, k1)
    assert.are.equal(30000, k2)
  end)
end)
...
describe("clamp_mired", function()
  it("passes through valid mired values", function()
    local m1 = Format.clamp_mired(100)
    local m2 = Format.clamp_mired(500)
    local m3 = Format.clamp_mired(1000)
  ...
  it("clamps out-of-range values", function()
    local m1 = Format.clamp_mired(10)
    local m2 = Format.clamp_mired(1500000)
...
describe("to_kelvin", function()
  ...
  it("clamps low mired values to 33", function()
    local kelvin = Format.to_kelvin(10)  -- Below minimum
    assert.equals(30303, kelvin)  -- 1000000 / 33 ≈ 30303
  end)

  it("clamps high mired values to 1000000", function()
    local kelvin = Format.to_kelvin(1200000)  -- Above maximum
