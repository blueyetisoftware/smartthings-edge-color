...
describe("Color module round trip tests", function()
  ...
  describe("XY round trip", function()
    ...
    it("XY -> RGB -> XY with tolerance", function()
      ...
      local recovered_x, recovered_y, _ = Color.rgb_to_xy(r, g, b)
      spec_helper.assert_near(original_x, recovered_x, 0.1)
      spec_helper.assert_near(original_y, recovered_y, 0.1)
    end)
  ...
  describe("Percentage round trip", function()
    it("Percentage -> from_percentage with tolerance", function()
      local original = 50.0
      local normalized = Color.from_percentage(original)
      local recovered = Color.to_percentage(normalized)
      spec_helper.assert_near(original, recovered, 0.01)
    end)
  end)

  describe("Degrees round trip", function()
    it("Degrees -> from_degrees with tolerance", function()
