...
describe("xyy_to_hsv", function()
  ...
  it("handles boundary xy (65535,65535)", function()
    local h, s, v = xyy_to_hsv(65535, 65535)
    assert.is_true(h >= 0 and h <= 1)
    assert.is_true(s >= 0 and s <= 1)
    assert.is_true(v >= 0 and v <= 1)
  end)
  ...
  it("defaults Y to 1.0 when omitted", function()
    local h1, s1, v1 = xyy_to_hsv(16206, 21550, 65535)  -- Y=1.0 in 16-bit
    local h2, s2, v2 = xyy_to_hsv(16206, 21550)         -- Y defaults to 1.0
